...
<section class="reservation-form">
    <form method="POST" action="{{ route('book-residential-suite.store') }}">
        @csrf

        {{-- Display validation errors and success messages as before --}}
        @if ($errors->any())
        ...
        <select name="hotel_branch" required>
        ...
        <div class="form-group">
            ...
            <div class="radio-buttons">
                <input type="radio" id="weekly-suite" name="suite_duration" value="weekly" {{ old('suite_duration') == 'weekly' ? 'checked' : '' }}>
                <label for="weekly-suite" class="radio-label">Weekly</label>
                <input type="radio" id="monthly-suite" name="suite_duration" value="monthly" {{ old('suite_duration') == 'monthly' ? 'checked' : '' }}>
                <label for="monthly-suite" class="radio-label">Monthly</label>
            </div>
        </div>

        <label>No. of Guests (Total)</label>
        <input type="number" name="num_guests" min="1" value="{{ old('num_guests') }}" required />
        ...
        <div class="card-details-section" id="card-details-section">
            ...
            {{-- Initially, these inputs will be enabled by default HTML, JS will disable/enable --}}
            ...
            <input type="text" id="card-exp" name="card_expiration" value="{{ old('card_expiration') }}" placeholder="MM/YY" />
...
{{-- Add the JavaScript at the end of the body or in a separate JS file --}}
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const bookingDataElement = document.getElementById('booking-data'); // Get the data element
        ...
        const oldCardNumber = bookingDataElement.dataset.oldCardNumber;
        ...
        const oldSuiteDuration = bookingDataElement.dataset.oldSuiteDuration;
        ...
        // Add event listeners for credit card buttons
        provideCreditBtn.addEventListener('click', showCardDetails);
        withoutCreditBtn.addEventListener('click', hideCardDetails);

        // =========================================================================
        // NEW JAVASCRIPT FOR "ADD ANOTHER ROOM" BUTTON - ADD THIS BLOCK
        // =========================================================================
